        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Non-Smooth Mechanics for Foundation Policies: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study classical mechanics and robot foundation models. The submission-hardened claim is intentionally narrow: Bring contact discontinuities into the policy objective instead of smoothing them away. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Non smooth mechanics for foundation policies should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.407 $\pm$ 0.015 & 0.440 $\pm$ 0.015 & 0.334 $\pm$ 0.014 \\
Data-augmented WAM & 0.500 $\pm$ 0.018 & 0.521 $\pm$ 0.012 & 0.422 $\pm$ 0.008 \\
Scalar uncertainty & 0.565 $\pm$ 0.013 & 0.566 $\pm$ 0.021 & 0.503 $\pm$ 0.017 \\
Ensemble disagreement & 0.596 $\pm$ 0.015 & 0.611 $\pm$ 0.018 & 0.553 $\pm$ 0.011 \\
Conformal risk filter & 0.622 $\pm$ 0.018 & 0.628 $\pm$ 0.011 & 0.565 $\pm$ 0.005 \\
Proposed branch mechanism & 0.683 $\pm$ 0.010 & 0.689 $\pm$ 0.013 & 0.666 $\pm$ 0.013 \\
Oracle branch upper bound & 0.766 $\pm$ 0.009 & 0.771 $\pm$ 0.010 & 0.755 $\pm$ 0.011 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.669 $\pm$ 0.013 & all components \\
Minus branch atlas & 0.556 $\pm$ 0.014 & removes explicit unrealized futures \\
Minus tail risk & 0.575 $\pm$ 0.007 & uses mean predicted value only \\
Minus diagnostic probe & 0.580 $\pm$ 0.013 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.619 $\pm$ 0.021 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/109.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Non-Smooth Mechanics for Foundation Policies is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
